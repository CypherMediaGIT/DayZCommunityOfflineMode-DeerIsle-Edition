class CameraSettings extends PopupMenu 
{
	...
	void Init()
    {
		...
		CAMERA_ROT = GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:missions\\DayZCommunityOfflineMode.deerisle\\core\\modules\\CameraTool\\gui\\layouts\\CameraROT.layout" );
		CAMERA_PHI = GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:missions\\DayZCommunityOfflineMode.deerisle\\core\\modules\\CameraTool\\gui\\layouts\\CameraPHI.layout" );
	}
