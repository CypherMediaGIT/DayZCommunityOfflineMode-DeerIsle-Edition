class CustomInGameMenu extends UIScriptedMenu
{
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets("$CurrentDir:missions\\DayZCommunityOfflineMode.deerIsle\\core\\modules\\UIExtender\\gui\\layouts\\CustomInGameMenu.layout");
		...
		if ( bottomButtonContainer )
		{
			...
			button = ButtonWidget.Cast( GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:missions\\DayZCommunityOfflineMode.deerIsle\\core\\modules\\UIExtender\\gui\\layouts\\BottomButtonTemplate.layout", bottomButtonContainer ) );
		...
		if ( topButtonContainer )
		{
			for ( int i = 1; i < m_OM.GetPauseButtons().Count(); i++ )
			{
				...
				button = ButtonWidget.Cast( GetGame().GetWorkspace().CreateWidgets( "$CurrentDir:missions\\DayZCommunityOfflineMode.deerIsle\\core\\modules\\UIExtender\\gui\\layouts\\TopButtonTemplate.layout", topButtonContainer ) );
